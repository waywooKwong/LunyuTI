...
# 从数据库中找角色在线生成
def process_news_data(request: GenerationRequest):
    # 初始化回答列表
    global answers
    # 初始化
    answers = []
...
# 相似度匹配
def similarity_news_match(request: GenerationRequest):
    """
    相似度匹配:新闻、自定义问题
    answer_from_front:前端返回的回答
    """
    user_answer = request.dialog  # 用户发表的见解
